    ...
    <div className="min-h-screen bg-cover bg-center bg-no-repeat text-white relative overflow-hidden"
      ...
      <div className="relative z-10 h-screen flex flex-col">
        ...
        {(formData.mosqueName || formData.location || formData.imam) && (
          <div className="flex justify-between items-center px-8 pb-2 w-full">
            {/* Mosque Info */}
            {(formData.mosqueName || formData.location) ? (
                  
                  <div className="text-green300 font-semibold text-xl">
                  
                      {formData.mosqueName && <>🕌 {formData.mosqueName}</>}
                      {formData.location && <span className="ml-2 text-blue-300 text-opacity-10"> - {formData.location}</span>}
                  </div> 
            ) : <div />}
            {/* Imam Info */}
            {formData.imam ? (
